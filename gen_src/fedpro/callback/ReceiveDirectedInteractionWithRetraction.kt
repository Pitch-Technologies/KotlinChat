...
// Source: fedpro.ReceiveDirectedInteractionWithRetraction in FederateAmbassador.proto
...
public class ReceiveDirectedInteractionWithRetraction(
  @field:WireField(
    tag = 1,
...
) : Message<ReceiveDirectedInteractionWithRetraction, Nothing>(ADAPTER, unknownFields) {
  @Deprecated(
  ...
  override fun equals(other: Any?): Boolean {
    ...
    if (other !is ReceiveDirectedInteractionWithRetraction) return false
  ...
  override fun toString(): String {
    ...
    return result.joinToString(prefix = "ReceiveDirectedInteractionWithRetraction{", separator =
        ", ", postfix = "}")
  }
  ...
  public fun copy(
    ...
    unknownFields: ByteString = this.unknownFields,
      ...
      ReceiveDirectedInteractionWithRetraction(interactionClass, theObject, theParameters,
      userSuppliedTag, sentOrdering, theTransport, theTime, receivedOrdering, retractionHandle,
      receiveInfo, unknownFields)

  public companion object {
    @JvmField
    public val ADAPTER: ProtoAdapter<ReceiveDirectedInteractionWithRetraction> = object :
        ProtoAdapter<ReceiveDirectedInteractionWithRetraction>(
      FieldEncoding.LENGTH_DELIMITED, 
      ReceiveDirectedInteractionWithRetraction::class, 
      "type.googleapis.com/fedpro.ReceiveDirectedInteractionWithRetraction", 
      PROTO_3, 
      null, 
      "FederateAmbassador.proto"
    ) {
      override fun encodedSize(`value`: ReceiveDirectedInteractionWithRetraction): Int {
        ...
        if (value.theTime != null) size += LogicalTime.ADAPTER.encodedSizeWithTag(7, value.theTime)
        if (value.receivedOrdering != OrderType.RECEIVE) size +=
            OrderType.ADAPTER.encodedSizeWithTag(8, value.receivedOrdering)
        if (value.retractionHandle != null) size +=
            MessageRetractionHandle.ADAPTER.encodedSizeWithTag(9, value.retractionHandle)
        if (value.receiveInfo != null) size +=
            SupplementalReceiveInfo.ADAPTER.encodedSizeWithTag(10, value.receiveInfo)
        return size
      }

      override fun encode(writer: ProtoWriter, `value`: ReceiveDirectedInteractionWithRetraction) {
        ...
        if (value.theTransport != null) TransportationTypeHandle.ADAPTER.encodeWithTag(writer, 6,
            value.theTransport)
        if (value.theTime != null) LogicalTime.ADAPTER.encodeWithTag(writer, 7, value.theTime)
        if (value.receivedOrdering != OrderType.RECEIVE) OrderType.ADAPTER.encodeWithTag(writer, 8,
            value.receivedOrdering)
        if (value.retractionHandle != null) MessageRetractionHandle.ADAPTER.encodeWithTag(writer, 9,
            value.retractionHandle)
        if (value.receiveInfo != null) SupplementalReceiveInfo.ADAPTER.encodeWithTag(writer, 10,
            value.receiveInfo)
      ...
      override fun decode(reader: ProtoReader): ReceiveDirectedInteractionWithRetraction {
        ...
        val unknownFields = reader.forEachTag { tag ->
          when (tag) {
            ...
            7 -> theTime = LogicalTime.ADAPTER.decode(reader)
            8 -> try {
              receivedOrdering = OrderType.ADAPTER.decode(reader)
            } catch (e: ProtoAdapter.EnumConstantNotFoundException) {
              reader.addUnknownField(tag, FieldEncoding.VARINT, e.value.toLong())
            }
            9 -> retractionHandle = MessageRetractionHandle.ADAPTER.decode(reader)
            10 -> receiveInfo = SupplementalReceiveInfo.ADAPTER.decode(reader)
            else -> reader.readUnknownField(tag)
          }
        }
        return ReceiveDirectedInteractionWithRetraction(
      ...
      override fun redact(`value`: ReceiveDirectedInteractionWithRetraction):
          ReceiveDirectedInteractionWithRetraction = value.copy(
