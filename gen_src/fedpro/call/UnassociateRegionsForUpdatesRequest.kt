// Code generated by Wire protocol buffer compiler, do not edit.
// Source: fedpro.UnassociateRegionsForUpdatesRequest in RtiAmbassador.proto
package fedpro.call

import com.squareup.wire.FieldEncoding
import com.squareup.wire.Message
import com.squareup.wire.ProtoAdapter
import com.squareup.wire.ProtoReader
import com.squareup.wire.ProtoWriter
import com.squareup.wire.ReverseProtoWriter
import com.squareup.wire.Syntax.PROTO_3
import com.squareup.wire.WireField
import com.squareup.wire.`internal`.JvmField
import fedpro.type.AttributeSetRegionSetPairList
import fedpro.type.ObjectInstanceHandle
import kotlin.Any
import kotlin.AssertionError
import kotlin.Boolean
import kotlin.Deprecated
import kotlin.DeprecationLevel
import kotlin.Int
import kotlin.Long
import kotlin.Nothing
import kotlin.String
import okio.ByteString

public class UnassociateRegionsForUpdatesRequest(
  @field:WireField(
    tag = 1,
    adapter = "fedpro.type.ObjectInstanceHandle#ADAPTER",
    label = WireField.Label.OMIT_IDENTITY,
    schemaIndex = 0,
  )
  public val theObject: ObjectInstanceHandle? = null,
  @field:WireField(
    tag = 2,
    adapter = "fedpro.type.AttributeSetRegionSetPairList#ADAPTER",
    label = WireField.Label.OMIT_IDENTITY,
    schemaIndex = 1,
  )
  public val attributesAndRegions: AttributeSetRegionSetPairList? = null,
  unknownFields: ByteString = ByteString.EMPTY,
) : Message<UnassociateRegionsForUpdatesRequest, Nothing>(ADAPTER, unknownFields) {
  @Deprecated(
    message = "Shouldn't be used in Kotlin",
    level = DeprecationLevel.HIDDEN,
  )
  override fun newBuilder(): Nothing = throw
      AssertionError("Builders are deprecated and only available in a javaInterop build; see https://square.github.io/wire/wire_compiler/#kotlin")

  override fun equals(other: Any?): Boolean {
    if (other === this) return true
    if (other !is UnassociateRegionsForUpdatesRequest) return false
    if (unknownFields != other.unknownFields) return false
    if (theObject != other.theObject) return false
    if (attributesAndRegions != other.attributesAndRegions) return false
    return true
  }

  override fun hashCode(): Int {
    var result = super.hashCode
    if (result == 0) {
      result = unknownFields.hashCode()
      result = result * 37 + (theObject?.hashCode() ?: 0)
      result = result * 37 + (attributesAndRegions?.hashCode() ?: 0)
      super.hashCode = result
    }
    return result
  }

  override fun toString(): String {
    val result = mutableListOf<String>()
    if (theObject != null) result += """theObject=$theObject"""
    if (attributesAndRegions != null) result += """attributesAndRegions=$attributesAndRegions"""
    return result.joinToString(prefix = "UnassociateRegionsForUpdatesRequest{", separator = ", ",
        postfix = "}")
  }

  public fun copy(
    theObject: ObjectInstanceHandle? = this.theObject,
    attributesAndRegions: AttributeSetRegionSetPairList? = this.attributesAndRegions,
    unknownFields: ByteString = this.unknownFields,
  ): UnassociateRegionsForUpdatesRequest = UnassociateRegionsForUpdatesRequest(theObject,
      attributesAndRegions, unknownFields)

  public companion object {
    @JvmField
    public val ADAPTER: ProtoAdapter<UnassociateRegionsForUpdatesRequest> = object :
        ProtoAdapter<UnassociateRegionsForUpdatesRequest>(
      FieldEncoding.LENGTH_DELIMITED, 
      UnassociateRegionsForUpdatesRequest::class, 
      "type.googleapis.com/fedpro.UnassociateRegionsForUpdatesRequest", 
      PROTO_3, 
      null, 
      "RtiAmbassador.proto"
    ) {
      override fun encodedSize(`value`: UnassociateRegionsForUpdatesRequest): Int {
        var size = value.unknownFields.size
        if (value.theObject != null) size += ObjectInstanceHandle.ADAPTER.encodedSizeWithTag(1,
            value.theObject)
        if (value.attributesAndRegions != null) size +=
            AttributeSetRegionSetPairList.ADAPTER.encodedSizeWithTag(2, value.attributesAndRegions)
        return size
      }

      override fun encode(writer: ProtoWriter, `value`: UnassociateRegionsForUpdatesRequest) {
        if (value.theObject != null) ObjectInstanceHandle.ADAPTER.encodeWithTag(writer, 1,
            value.theObject)
        if (value.attributesAndRegions != null)
            AttributeSetRegionSetPairList.ADAPTER.encodeWithTag(writer, 2,
            value.attributesAndRegions)
        writer.writeBytes(value.unknownFields)
      }

      override fun encode(writer: ReverseProtoWriter,
          `value`: UnassociateRegionsForUpdatesRequest) {
        writer.writeBytes(value.unknownFields)
        if (value.attributesAndRegions != null)
            AttributeSetRegionSetPairList.ADAPTER.encodeWithTag(writer, 2,
            value.attributesAndRegions)
        if (value.theObject != null) ObjectInstanceHandle.ADAPTER.encodeWithTag(writer, 1,
            value.theObject)
      }

      override fun decode(reader: ProtoReader): UnassociateRegionsForUpdatesRequest {
        var theObject: ObjectInstanceHandle? = null
        var attributesAndRegions: AttributeSetRegionSetPairList? = null
        val unknownFields = reader.forEachTag { tag ->
          when (tag) {
            1 -> theObject = ObjectInstanceHandle.ADAPTER.decode(reader)
            2 -> attributesAndRegions = AttributeSetRegionSetPairList.ADAPTER.decode(reader)
            else -> reader.readUnknownField(tag)
          }
        }
        return UnassociateRegionsForUpdatesRequest(
          theObject = theObject,
          attributesAndRegions = attributesAndRegions,
          unknownFields = unknownFields
        )
      }

      override fun redact(`value`: UnassociateRegionsForUpdatesRequest):
          UnassociateRegionsForUpdatesRequest = value.copy(
        theObject = value.theObject?.let(ObjectInstanceHandle.ADAPTER::redact),
        attributesAndRegions =
            value.attributesAndRegions?.let(AttributeSetRegionSetPairList.ADAPTER::redact),
        unknownFields = ByteString.EMPTY
      )
    }

    private const val serialVersionUID: Long = 0L
  }
}
